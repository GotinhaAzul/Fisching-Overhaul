...
def test_show_market_appraise_excludes_rod_exclusive_mutations_characterization(monkeypatch) -> None:
    starter, premium = _make_rods()
    selected_pool, fish = _make_pool_and_fish()
    equipped_rod = Rod(
        name="Promessa Luminescente",
        luck=0.0,
        kg_max=100.0,
        control=0.0,
        description="Exclusive rod",
        price=0.0,
        unlocked_default=True,
    )
    entry = InventoryEntry(name=fish.name, rarity=fish.rarity, kg=2.0, base_value=fish.base_value)
    ...
    def _capture_mutation_pool(mutations: list[Mutation]) -> Mutation:
        captured_pool[:] = [mutation.name for mutation in mutations]
        return global_mutation

    monkeypatch.setattr(market, "clear_screen", lambda: None)
    monkeypatch.setattr(market.random, "uniform", lambda _a, _b: 2.7)
    monkeypatch.setattr(market, "choose_mutation", _capture_mutation_pool)
    monkeypatch.setattr("builtins.input", _InputFeeder(["4", "1", "t", "0", "0"]))
    ...
    balance, level, xp = market.show_market(
        ...
        level=1,
        xp=0,
        available_rods=[starter, premium, equipped_rod],
        owned_rods=[starter, equipped_rod],
        fish_by_name={fish.name: fish},
        available_mutations=[global_mutation, exclusive_mutation],
